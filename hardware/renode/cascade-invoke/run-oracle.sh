...
[ "${#N1[@]}" -ge 5 ] || fail "NC1 produced no result block"
same=1; for i in 1 2 3 4; do [ "${N1[$i]}" = "${W[$i]}" ] || same=0; done
...
if [ "${#N2[@]}" -ge 5 ]; then
  match=1; for i in 1 2 3 4; do [ "${N2[$i]}" = "${W[$i]}" ] || match=0; done
...
echo
echo "PASS — a falcon cascade stage EXECUTED on emulated RT1176 Cortex-M7 and reproduced"
echo "       relay's SIL reference bit-exact; a perturbed input moves it, and removing the"
echo "       embedder init breaks it."
